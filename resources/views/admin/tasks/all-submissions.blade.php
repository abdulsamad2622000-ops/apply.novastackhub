...
<div style="display:flex; gap:16px; margin-bottom:16px; flex-wrap:wrap;">
    <div class="form-group" style="max-width:260px;">
        ...
        <select onchange="window.location.href = this.value ? '{{ route('admin.submissions.index') }}?task_id=' + this.value : '{{ route('admin.submissions.index') }}'">
            <option value="">All Tasks</option>
            @foreach ($tasks as $t)
                <option value="{{ $t->id }}" {{ (string) $activeTaskId === (string) $t->id ? 'selected' : '' }}>{{ $t->title }}</option>
            @endforeach
        </select>
    </div>
...
<table class="admin-table">
    ...
    <tbody>
        ...
        @empty
            <tr>
                <td colspan="8" class="muted">Abhi koi submission nahi aayi.</td>
            </tr>
        @endforelse
    </tbody>
</table>
